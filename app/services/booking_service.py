...
def get_experiment_types():
    return db.session.execute(
        text("SELECT experiment_id, name FROM ExperimentTypes")
    ).fetchall()
...
def get_available_rooms(
    lab_zone_id=None, experiment_id=None, date=None, start_time=None, end_time=None
):
    query = text(
...
def has_overlapping_booking(user_id, date, selected_slots):
    ...
    return db.session.execute(
        query,
        {"user_id": user_id, "date": date, "selected_slots": list(selected_slots)},
    ).fetchone()
...
def is_room_already_booked(room_id, date, selected_slots):
    ...
    return db.session.execute(
        query,
        {"room_id": room_id, "date": date, "selected_slots": list(selected_slots)},
    ).fetchone()
...
def create_room_booking(user_id, room_id, experiment_id, date, selected_slots):
    try:
        lab_zone_query = text(
            "SELECT lab_zone_id FROM LabRooms WHERE lab_room_id = :room_id"
        )
        lab_zone_id = db.session.execute(lab_zone_query, {"room_id": room_id}).scalar()

        sorted_slots = sorted(
            [datetime.strptime(slot, "%H:%M:%S") for slot in selected_slots]
        )
...
def cancel_room_booking(reservation_id, user_id):
    try:
        ...
        result = db.session.execute(
            check_query, {"reservation_id": reservation_id}
        ).fetchone()
